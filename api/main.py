...
intel_store = []
DOMAINS = ["darkforum-alpha.onion", "pastebin-mirror.net", "leak-registry.io", "shadow-exchange.xyz", "threat-intel-hub.net"]
THREAT_ACTORS = ["APT-28", "Lazarus Group", "Cozy Bear", "Fancy Bear", "SilverFish", "Unknown Actor"]
ATTACK_VECTORS = ["Phishing", "Supply Chain", "Zero-Day Exploit", "Credential Stuffing", "Ransomware", "Social Engineering"]
TARGETS = ["Financial Sector", "Government Infrastructure", "Healthcare", "Energy Grid", "Tech Startups", "Academic Institutions"]
...
def generate_intel():
    return {
        ...
        "discovered_at": (datetime.utcnow() - timedelta(hours=random.randint(0, 72))).isoformat(),
...
@app.get("/")
def root():
    return {"status": "ONLINE", "platform": "PhantomGrid-OSINT", "intel_entries": len(intel_store)}
...
def list_actors():
    ...
    for a in actor_map:
        actor_map[a]["avg_severity"] = round(actor_map[a]["avg_severity"] / actor_map[a]["count"], 2)
...
def get_alerts(severity_min: int = 7):
    ...
    return {"critical_alerts": len(alerts), "alerts": sorted(alerts, key=lambda x: x["severity"], reverse=True)}

@app.get("/api/v1/ioc")
def ioc_feed():
    iocs = [{"ioc": i["ioc"], "type": "IPv4", "severity": i["severity"], "ttps": i["ttps"]} for i in intel_store]
